...
public class Board {
    private boolean black_to_move = false;
    ...
    public Board(Piece[][] board, Piece[] black, Piece[] white) {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                ...
                this.board[i][j] = new Piece(board[i][j]);              // make a deep copy
            }
        }
    ...
    public boolean move(cord cur, cord dest) throws Exception { // if we move return true and moves else return false and does nothing
        if(extract(cur).isBlack() != black_to_move){
            System.out.println("over side to move");
            return false;
        }
        ...
        if (flag || castle) { // now we need to see if the king is attacked
            if (extract(cur).isBlack()) { // black to move
                ...
                if (castle) {
                    if (!isClear(cur, dest)){ System.out.println("the path is not clear");return false;}
                ...
                if (!temp.Wcheck()) {
                    ...
                    System.out.println("black is being checked");
                }
            } else {                                   //white to move
                Board temp = new Board(this.board, this.black, this.white); // we got temporary board now
                if (castle) {
                    if (!isClear(cur, dest)){
                        System.out.println("the path is not clear"); return false;}
                    temp.castle(cur, dest);
                } else {
                    temp.remove(cur, dest, start);
                }        //move the piece and now check if our king is attacked
        ...
        System.out.println("can't do that move");
    ...
    public boolean Bcheck() throws Exception { // we need to know who we are even checking
        ...
        for (Piece piece : black) { //white being checked
           if(piece==null||piece.getCord() == null||!piece.alive()){continue;}
    ...
    LinkedList<cord> rock(cord cur) throws Exception {
        ...
        if(extract(cur)==null)return rocker;
    ...
    LinkedList<cord> bishof(cord cur) throws Exception {
        ...
        if(extract(cur)==null)return bisher;

        for (int i = 0; i < arr.length; i++) {
            cord here = new cord(cur);
            here.add(arr[i]);
            while (inBoard(here) && (extract(here) == null)) {
                bisher.add(new cord(here));
                here.add(arr[i]);
            }
            if(inBoard(here)) {
                if(extract(here)==null){
                ...
                else if(!same_color(here, cur)) { // I did here try and catch for some reason, not sure why
                    bisher.add(here);
                }
            }
        }
    ...
    LinkedList<cord> pawn(cord cur) {
        ...
        if (!extract(cur).isBlack()) {// then it's white we look from downwards up!
            ...
            } else if (inBoard(right) && extract(right)!= null) { ///////////////////////en pasent doesn't work///////////////////////
    ...
    LinkedList<cord> King(cord cur) throws Exception {
        ...
        for (int i = cur.getX()-1; i <= cur.getX()+1; i++) {
            for (int j = cur.getY()-1; j <= cur.getY()+1; j++) {
                ...
                if(inBoard(here)){
                    ...
                    if(extract(here) == null || !same_color(here,cur)){
                        lesbian.add(new cord(here));
                    }
                }
            }
        }
...
//    }
    ...
    public Piece extract(cord here) {
        if (!inBoard(here)) {
            return null;
        }
